...
/** A Merkle Patricia Forestry (MPF) is a key:value structure which stores elements in a radix trie following a key, and
  ...
  *
  * This library enforces (through hashing) that we use trie of radix 16 (hexadecimal alphabet). This means that each
  * level in the trie has up to 16 branches.
  *
  * An MPF allows for checking membership, insertion and deletion in the trie using only root hashes and a succinct
  * proofs. They are quite efficient in both cpu and mem units. And they also provide proofs that are a / lot smaller
  * than traditional Merkle Patricia Trie; proofs remain however the / main limiting factor.
  */
...
object MerklePatriciaForestry:
    ...
    /** Types of proof steps that can occur in a proof Each step contains a skip value indicating length of common
    ...
    /** A proof is a list of steps processed left to right, corresponding to neighbor nodes along the path to the
    ...
    extension (self: MerklePatriciaForestry)
        ...
        def insert(key: ByteString, value: ByteString, proof: Proof): MerklePatriciaForestry =
            require(MerklePatriciaForestry.excluding(key, proof) == self.root, "Invalid proof or element exists")
